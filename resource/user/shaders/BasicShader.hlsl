...
static float s_specularCol;
static const float s_emissiveBrightThreshold = 0.2f;
...
float3 BRDF(float3 LigDirection, float3 LigColor, float3 WorldNormal, float3 WorldPos, float3 EyePos)
{
    ...
    float3 specularColor = CookTorranceSpecular(NdotL, NdotV, NdotH, LdotH) * LigColor * s_specularCol;
...
PSOutput PSmain(VSOutput input) : SV_TARGET
{
    ...
    s_specularCol = float3(0.243f, 0.854f, 0.890f);
