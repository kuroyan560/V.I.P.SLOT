...
void GaussianBlur::GeneratePipeline(std::array<std::shared_ptr<GraphicsPipeline>, PROCESS_NUM - 1>& arg_destPipeline, const DXGI_FORMAT& arg_format)
{
...
GaussianBlur::GaussianBlur(const Vec2<int>& arg_size, const DXGI_FORMAT& arg_format, const float& arg_blurPower)
{
...
void GaussianBlur::SetBlurPower(const float& arg_blurPower)
{
    // 重みの合計を記録する変数を定義する
    float total = 0;
...
#include"DrawFunc2D.h"
void GaussianBlur::Register(const std::shared_ptr<TextureBuffer>& arg_srcTex)
{
